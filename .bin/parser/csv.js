...
(function (csv) {
     ...
     */
    function escape(s, sy) {
        return s.replaceAll(new RegExp(sy.dQuotes, "g"), `${sy.dQuotes}${sy.dQuotes}`);
    }
    /**
     ...
     */
    function unescape(s, sy) {
        return s.replaceAll(new RegExp(`${sy.dQuotes}${sy.dQuotes}`, "g"), sy.dQuotes);
    }
    /**
     ...
     */
    function hasIllegalWhitespaces(s, isWhitespace) {
     ...
     */
    function isQuoted(s, isWhitespace, sy) {
     ...
     */
    function split(st, s, store) {
        if (st.length === 0)
            return;
        let i = 1;
        while (i < st.length) {
            if (st[i] === s.nop || st[i] === s.nap)
                break;
            i++;
        }
     ...
     */
    function isHeader(data, row, s, p) {
        if (row > 1)
            return false;
    ...
    class SyntaxBuilder {
         ...
         * @defaultValue `"utf-8"`
         */
        _md = { encoding: "utf-8", fileExt: "csv", isStandard: true, mediaType: "text/csv", standard: "Rfc 4180" };
        /**
        ...
        #anyWhitespaces(f, props) {
            for (let i = 0; i < props.length; i++) {
                if (f(props[i]))
                    throw new TypeError("A whitespace cannot include a token");
            }
        }
        /**
        ...
        setIsWhiteSpace(isWhitespace) {
            if (!utility.isValid(isWhitespace))
                return this;
        ...
        setEol(lt) {
            ...
            for (let i = 0; i < props.length; i++) {
                if (props[i] === lt)
                    throw new TypeError("line terminator conflicts with other properties");
            }
            if (this.#isWs(lt))
        ...
        setSeparator(s) {
            ...
            for (let i = 0; i < props.length; i++) {
                if (props[i] === s)
                    throw new TypeError("separator conflicts with other properties");
            }
            if (this.#isWs(s))
        ...
        setDQuotes(dq) {
            ...
            for (let i = 0; i < props.length; i++) {
                if (props[i] === dq)
                    throw new TypeError("quotes conflicts with other properties");
            }
            if (this.#isWs(dq))
        ...
        clear() {
            ...
            this.#posCmdlets = [];
            this.#dQuote = '"';
            this._md = { encoding: "utf-8", fileExt: "csv", isStandard: true, mediaType: "text/csv", standard: "Rfc 4180" };
            this.#bom = false;
            this.#nop = ".";
            this.#nap = "#";
            this.#p = (s) => s.length > 0 ? s : null;
            this.#getCmd = (d, type) => {
                switch (d) {
                    case parser.Direction.PREFIX:
                    default: {
    ...
    csv.SyntaxBuilder = SyntaxBuilder;
    /**A mutative object used to store temporay data about a document or an expression */
    class Params {
        #header; //a list of string naming the headers
        #dih; //doc is headerless
        /**
    ...
    class Type {
        ...
        toString() {
            return JSON.stringify({ id: this.id, precedence: this.precedence }, null, 2);
        }
    }
    /**
     ...
     * @readonly
     */
    csv.EOF = new Type("-1", Number.MIN_SAFE_INTEGER);
    /**
    ...
    class JSONLexer {
        ...
        #extractHeaders({ chunk, s, doc, h, row, col, from, }) {
            if (json.isAtomic(chunk)) {
                ...
                for (let i = 0; i < chunk.length; i++) {
                    if (!from &&
                        ...
                        typeof chunk[i] === "object")
                        this.#extractHeaders({ chunk: chunk[i], s, doc, h, row: i, col });
                    else if (s.nap) {
        ...
        #applyHeaders(value, s, p, tree) {
            if (typeof tree === "string") {
                ...
                if (tree.length === 0) {
                    if (json.isAtomic(value))
                        return this.process(value ?? null, s, p);
                    else
                        return this.process(JSON.stringify(value ?? null), s, p);
                }
                let child = fixFieldName(tree.shift(), s);
                ...
                value = utility.isValid(value)
                    ...
                    : typeof child === "string"
                        ? {}
                        : [];
                return this.#applyHeaders(value[child], s, p, tree);
            }
        ...
        process(chunk, syntax, p) {
            ...
            if (chunk === null) {
                //null value
                this.#manufacture(new Token("", csv.FIELD, this.line(), this.line(), this.position()));
                return;
            }
            else {
                ...
                switch (typeof chunk) {
                    ...
                    case "number":
                    case "string": {
                        this.#manufacture(new Token(`${chunk}`, csv.FIELD, this.line(), this.line(), this.position()));
                        return;
                    }
                    case "object": {
                        ...
                        if (!p.header) {
                            ...
                            for (this.#row = 0; this.#row < this.#doc.length; this.#row++) {
                                ...
                                for (this.#col = 0; this.#col < this.#h.length; this.#col++) {
                                    this.#manufacture(new Token(row[this.#col] ?? "", csv.FIELD, this.line(), this.line(), this.position()));
                                    if (this.#col < this.#h.length - 1)
                                        this.#manufacture(new Token(syntax.separator, csv.SEPARATOR, this.#row, this.#row, this.#col));
                                }
                                this.#manufacture(new Token(syntax.eol, csv.EOL, this.#row, this.#row, this.#col));
                            }
                        }
                        else {
                            ...
                            for (this.#row = 0; this.#row < l; this.#row++) {
                                for (this.#col = 0; this.#col < p.header.length; this.#col++) {
                                    ...
                                    if (this.#col < p.header.length - 1)
                                        this.#manufacture(new Token(syntax.separator, csv.SEPARATOR, this.line(), this.line(), this.position()));
                                }
                                this.#manufacture(new Token(syntax.eol, csv.EOL, this.#row, this.#row, this.#col));
                            }
                        }
                ...
                if (!p.header) {
                    ...
                    for (this.#row = 0; this.#row < chunk.length; this.#row++) {
                        for (this.#col = 0; this.#col < p.header.length; this.#col++) {
                            this.#applyHeaders(chunk[this.#row], syntax, p, p.header[this.#col]);
                            if (this.#col < p.header.length - 1)
                                this.#manufacture(new Token(syntax.separator, csv.SEPARATOR, this.line(), this.line(), this.position()));
                        }
                        this.#manufacture(new Token(syntax.eol, csv.EOL, this.line(), this.line(), this.position()));
                    }
                }
            }
            this.#src = chunk;
        ...
        next() {
            ...
            return new Token("", csv.EOF, this.line(), this.line(), this.position());
        }
        position() {
            return this.#col;
        }
    ...
    csv.JSONLexer = JSONLexer;
    class StringLexer {
        // #lqt?: Token;//last queried token
        #ln; //the current line being being read from. This value is reset each time a line terminator is encountered outside of a quoted string.
        #li; //the character's position in the line that is being analysed. This value is reset each time a new character is being analysed
        // #token?: string;//the text currently being analysed as a token
        #header; //an array of the header columns
        src; //an array characters read from a stream
        #field; //the current field value. When this prop's length at least 1, there a field is being read
        #op; //the current field value. When this prop's length at least 1, there a field is being read
        #queue; //an array of already-made token objects
        constructor() {
            this.#ln = 1;
            this.#li = 0;
            this.#header = [];
        ...
        #inQuote(field, s) {
            ...
            return (stripEscaped.startsWith(s.dQuotes) && !stripEscaped.endsWith(s.dQuotes));
        }
        #splitChunk(chunk) {
        ...
        // end(syntax: Syntax, params: unknown): void;
            ...
            if (this.#queue[this.#queue.length - 1] && !this.#queue[this.#queue.length - 1].type.equals(csv.EOL))
         ...
         */
        processed = () => Object.freeze(this.#queue);
        frequency(type) {
            let frqy = 0;
            for (let i = 0; i < this.#queue.length; i++) {
                if (this.#queue[i].type.equals(type))
        ...
        process(chunk, syntax, params) {
            params.headerless = this.line() < 1 && utility.isValid(params.header);
            if (chunk.length > 0) {
                const string = this.#splitChunk(chunk);
                if (Array.isArray(string))
                    string.forEach((x) => this.src.push(x));
                else
                    this.src.push(string);
            }
            while (this.src.length > 0) {
                const token = this.src.shift();
                this.#li++;
                if (syntax.eol.startsWith(this.#op + token) &&
                    !this.#inQuote(this.#field, syntax)) {
                    this.#op += token;
                    if (this.#op === syntax.eol) {
                        this.#manufacture(new Token(this.#field, csv.FIELD, this.line(), this.line(), this.position() - this.#field.length - this.#op.length));
                        if (!params.header) {
                        ...
                        this.#field = "";
                        this.#manufacture(new Token(this.#op, csv.EOL, this.line(), this.line(), this.position() - this.#op.length));
                        this.#li = 0;
                        this.#ln++;
                        this.#op = "";
                    }
                }
                else if (syntax.separator.startsWith(this.#op + token) &&
                    !this.#inQuote(this.#field, syntax)) {
                    this.#op += token;
                    if (this.#op === syntax.separator) {
                        ...
                        this.#manufacture(new Token(this.#field, csv.FIELD, this.line(), this.line(), this.position() - this.#field.length - this.#op.length));
                        this.#field = "";
                        this.#manufacture(new Token(this.#op, csv.SEPARATOR, this.line(), this.line(), this.position() - this.#op.length));
                        this.#op = "";
                    }
                }
                else {
        ...
        parse(ap, yp, p, l, s, pa) {
            ...
            if (s.quotesType !== "none" &&
                ...
                value = s.trimLeadingSpaces
                    ...
                    : value;
                return new QuotedExpression(s.dQuotes, new FieldExpression(unescape(value, s)), s.dQuotes);
            }
    ...
    class ParseSeparator {
        parse(ap, yp, p, l, s, pa) {
            ...
            return new SeparatorExpression(ap, yp.value, p.parseWithPrecedence(precedence, l, s, pa));
        }
    }
    class ParseRow {
        static #getString(ap, s, p) {
        ...
        parse(ap, yp, p, l, s, pa) {
            if (s.enfSym && pa?.fieldCount !== pa?.header?.length) {
                ...
                const msg = `Misaligned row. row must have exactly ${pa?.header?.length} field(s) but ${pa?.fieldCount} field(s) was found at row ${pa.rowCount}, column ${pa.fieldCount}.${s.eol}This is the affected data: ${affectedRow}${s.eol}`;
    ...
    class QuotedExpression {
        ...
        content;
        close;
        constructor(open, content, close) {
            this.open = open;
        ...
        equals(obj) {
            if (obj instanceof QuotedExpression)
                return this.compareTo(obj) === 0;
            return false;
        }
        hashCode32() {
            return utility.hashCode32(false, utility.asHashable(this.open), this.content, utility.asHashable(this.close));
        }
        toString() {
            return this.debug();
        }
    ...
    class SeparatorExpression {
        ...
        hashCode32() {
            return utility.hashCode32(false, this.pre, utility.asHashable(this.sep), this.post);
        }
        toString() {
            return this.debug();
        }
    ...
    class RecordExpression {
        ...
        equals(obj) {
            if (obj instanceof RecordExpression)
                return this.compareTo(obj) === 0;
            return false;
        }
        hashCode32() {
    ...
    class StringFormat {
        ...
        bpn;
        hashCode32() {
            return utility.hashCode32(false, utility.asHashable(this.#value), utility.asHashable(this.#row), utility.asHashable(this.#col), utility.asHashable(this.modifications), utility.asHashable(this.bpc), utility.asHashable(this.bpn));
        }
        toJSON() {
    ...
    class JSFormat {
        ...
        #av(v, s, parent, col) {
            if (typeof col === "string") {
                ...
                if (col.length > 0) {
                    if (col[0][0] === s.nop && parent[n] === undefined) {
                        parent[n] = {};
                    }
                    else if (col[0][0] === s.nap && parent[n] === undefined) {
                        parent[n] = [];
                    }
                    return this.#av(v, s, parent[n], col);
                }
                else {
                    //prevent overwrite
                    if (!parent[n])
        ...
        bpn = 32;
        hashCode32() {
            return utility.hashCode32(false, utility.asHashable(this.modifications), utility.asHashable(this.bpc), utility.asHashable(this.bpn), utility.asHashable(this.#col), utility.asHashable(this.#row), utility.asHashable(this.#value));
        }
        toJSON() {
    ...
    class FileFormat {
        ...
        append(data, s, p) {
            if (!utility.isValid(this.#ws)) {
                ...
                this.#ws = fs.createWriteStream(this.#fs, this.#enc);
                if (!this.#bw && s.bom) {
        ...
        hashCode32() {
            return utility.hashCode32(false, utility.asHashable(this.modifications), utility.asHashable(this.bpc), utility.asHashable(this.bpn), utility.asHashable(this.#fs), utility.asHashable(this.#ws));
        }
        toJSON() {
            return "";
        }
    ...
    class Converter extends parser.Converter {
        _transform(chunk, encoding, callback) {
            ...
            if (this.lexer.hasTokens()) {
                while (this.lexer.indexOf(csv.EOL) >= 0)
                    try {
     ...
     */
    csv.RFC_4180 = new SyntaxBuilder()
        .addPrefixCommand(csv.FIELD, new ParseField())
        // .addPrefixCommand(QUOTE, new QuotedCommand())
        .addInfixCommand(csv.SEPARATOR, new ParseSeparator())
        .addInfixCommand(csv.EOL, new ParseRow())
