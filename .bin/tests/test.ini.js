...
const path = resolve(fileURLToPath(new URL(".", import.meta.url).toString()), "./test.ini");
// lexer test
const rs = createReadStream(path);
rs.pipe(converter).on("data", (chunk) => {
    chunk.format(format, syntax, params);
    chunk.format(format2, syntax, params);
})
    .on("end", () => {
