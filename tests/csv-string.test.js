import csv from "./.bin/parser/csv.js";
import parser from "./.bin/parser/parser.js";
const stock = `Index,Organization Id,Name,Website,Country,Description,Founded,Industry,Number of employees
...
// const s = csv.RFC_4180;
const s = new csv.SyntaxBuilder()
.rebuild(csv.RFC_4180)
.setIsWhiteSpace(s => s === ' ')
.setEol("\n")
.build();
// const l = new csv.CSVLexer(`"name","age","hei""ght",""," desc"\r\n"tine",25,5.9,false,"A ,competent ""mathaid"" engineer"`, s);
const l = new csv.CSVLexer(stock);
// const l = new csv.CSVLexer('"""""desc"""""');
const pr = new csv.CSVParser();
const p = new csv.CSVParams();
// let t;
// while (!(t = l.next(s, p)).type.equals(csv.EOF)) {
//     console.log(t);
// }

const f = new csv.JSFormat();

while(true) {
    try {
        const exp = pr.parse(l, s, p);
        exp.format(f, p, s);
    } catch (e) {
        console.error(e);
        break;
