...
namespace ini {
  ...
  function processEscapables(text: string, s: Syntax, f: Format): string {
    ...
    for (let i = 0; i < text.length; i++) {
      let isEscapable;
      if (!Array.isArray(s.escape!.isSpecial))
        isEscapable = (s.escape!.isSpecial as (e: string) => boolean)(text[i]);
      else isEscapable = s.escape!.isSpecial.indexOf(text[i]) >= 0;

      if (isEscapable) {
        const c = text[i].codePointAt(0)!.toString(16);
        const escaped =
          s.escape!.unicode.length > 0
            ? `${s.escape!.char}${s.escape!.unicode[0]}${utility.chars(
                "0",
                4 - c.length
              )}${c}`
            : text[i];
        if (s.escape!.quoted) {
          ...
          if (utility.isValid(f.logger)) {
            f.logger!.warn(
              `The character '${text[i]}' in string "${text}" needs to be escaped but it's not because the syntax does not support escaping special characters. The next time it is parsed from this format, an error will be thrown`
            );
          }
          val += text[i];
        }
  ...
  function append(f: Format<any>, data: Section, s?: Syntax, p?: Params) {
    for (const key in data.map) {
      if (data.map[key] instanceof Section) {
        f.append(
          data.map[key].comments.preceding.length > 0
            ? unwrapComments(data.map[key].comments, s)
            : "",
          s,
          p
        );
        if ((data.map[key] as Section).fullname.length > 0)
          f.append(
            `${s!.sectionOperators[0]}${(
              data.map[key] as Section
            ).fullname.join((s!.nesting ?? { chars: [""] }).chars[0])}${
              s!.sectionOperators[1]
            }\n`
          );
  ...
  function unwrapComments(
    comments: { preceding: readonly string[] },
    s?: Syntax
  ) {
  ...
  function skipBlankLines(
    ...
    let x = "";
    while (true) {
      if (p.match(EOL, l, s, pa)) x += p.consume(EOL, l, s, pa).value;
  ...
  function parseComment(
    ...
    while (true) {
      if (p.match(COMMENT, l, s, pa)) c += p.consume(COMMENT, l, s, pa).value;
        ...
        c += p.consume(SECTION_START, l, s, pa).value;
      else if (p.match(D_QUOTE, l, s, pa))
        c += p.consume(D_QUOTE, l, s, pa).value;
      else if (p.match(QUOTE, l, s, pa)) c += p.consume(QUOTE, l, s, pa).value;
      else if (p.match(IDENTIFIER, l, s, pa))
        c += p.consume(IDENTIFIER, l, s, pa).value;
      else if (p.match(SUB_SECTION, l, s, pa))
        c += p.consume(SUB_SECTION, l, s, pa).value;
      else if (p.match(SECTION_END, l, s, pa))
        c += p.consume(SECTION_END, l, s, pa).value;
      else if (p.match(ASSIGNMENT, l, s, pa))
        c += p.consume(ASSIGNMENT, l, s, pa).value;
      else if (p.match(D_QUOTE_END, l, s, pa))
        c += p.consume(D_QUOTE_END, l, s, pa).value;
      else if (p.match(QUOTE_END, l, s, pa))
        c += p.consume(QUOTE_END, l, s, pa).value;
      else if (p.match(ESCAPE, l, s, pa))
        c += p.consume(ESCAPE, l, s, pa).value;
      else if (p.match(ESCAPED, l, s, pa))
        c += p.consume(ESCAPED, l, s, pa).value;
      else if (p.match(WHITESPACE, l, s, pa))
        c += p.consume(WHITESPACE, l, s, pa).value;
   ...
   */
  function comments(
    preceding = Object.freeze(Array<string>()),
    inline: string = ""
  ) {
  ...
  export class SyntaxBuilder implements utility.Builder<Syntax> {
    ...
    private _com: { retain: boolean; chars: string[]; inline: boolean } = {
      retain: true,
      chars: [";", "#"],
      inline: true,
    };
    ...
    private _nes?: { chars: string[]; relative: boolean } = {
      chars: [".", "/"],
      relative: true,
    };
    ...
    private _esc?: {
      quoted: boolean;
      nonQuotedEsc: boolean;
      unicode: string[];
      char: string;
      isSpecial: ((s: string) => boolean) | string[];
    ...
    private _p: (v: string) => json.Value = (v: string) =>
      v.length > 0 ? v : null;
    ...
    private _ensureUniqueness(s: string, truthy?: any): asserts s is string {
      if (!utility.isValid(s)) throw Error("undefined cannot be used");
        ...
        typeof s !== "string" ||
        (typeof s === "object" && !((s as String) instanceof String))
      )
        throw Error("only strings can be used");
      ...
      if (utility.isValid(this._esc))
        if (
          Array.isArray(this._esc!.isSpecial) &&
          this._esc!.isSpecial.indexOf(s) >= 0
        )
      ...
      if (utility.isValid(this._esc) && this._esc!.char === s)
        throw new Error(
          `${s} is not unique. It is used as the escape character`
        );
    }
    /**
     ...
     */
    public setDuplicateDirective(
      dd: DuplicateDirective,
      forProperty?: boolean
    ): SyntaxBuilder {
    ...
    private _pushOrOverite(
      map: [parser.GType<string>, Command][],
      t: parser.GType<string>,
      cmd: Command
    ): void {
    ...
    public clear(toDefault = true): SyntaxBuilder {
      ...
      if (toDefault) {
        ...
        this._dd = {
          section: DuplicateDirective.THROW,
          property: DuplicateDirective.THROW,
        };
        this._del = [];
    ...
    public build(): Syntax {
      if (utility.isValid(this._esc)) {
        ...
        if (!utility.isValid(this._esc!.isSpecial))
          this._esc!.isSpecial = (s: string) =>
            ["\n", "\r", '"', "'"].indexOf(s) >= 0 ||
            this._so.indexOf(s) >= 0 ||
            this._com.chars.indexOf(s) >= 0 ||
            this._del.indexOf(s) >= 0 ||
            this._esc!.char === s;
        if (!utility.isValid(this._esc!.parse))
          this._esc!.parse = (e: string) => {
            ...
            if (this._com.chars.indexOf(esc) && e.length === 2) return esc;
              ...
              utility.isValid(this._nes) &&
              this._nes!.chars.indexOf(esc) &&
              e.length === 2
            )
              return esc;
      ...
      return Object.freeze({
        ...
        nesting: utility.isValid(this._nes)
          ? { ...this._nes!, chars: Object.freeze(this._nes!.chars) }
          : undefined,
        duplicateDirective: Object.freeze(this._dd),
        escape: utility.isValid(this._esc)
          ? { ...this._esc!, unicode: Object.freeze(this._esc!.unicode) }
          : undefined,
        parse: this._p,
    ...
    public rebuild(from: Syntax): SyntaxBuilder {
      ...
      //escape
      (this._esc ??= {} as any).nonQuotedEsc = (
        from.escape ?? { nonQuotedEsc: false }
      ).nonQuotedEsc;
      (this._esc ??= {} as any).quoted = (
        from.escape ?? { quoted: false }
      ).quoted;
      (this._esc ??= {} as any).char = (from.escape ?? { char: "" }).char;
      (this._esc ??= {} as any).unicode = [
        ...(from.escape ?? { unicode: [] }).unicode,
      ];
      (this._esc ??= {} as any).isSpecial = (
        from.escape ?? { isSpecial: [] }
      ).isSpecial;
      (this._esc ??= {} as any).parse = (
        from.escape ?? { parse: (s: string) => s }
      ).parse;

      //nesting
      (this._nes ??= {} as any).chars = [
        ...(from.nesting ?? { chars: [] }).chars,
      ];
      (this._nes ??= {} as any).relative = (
        from.nesting ?? { relative: false }
      ).relative;
   ...
   * @readonly
   */
  export const EOF: parser.GType<string> = new Type(
    "-1",
   ...
   * @readonly
   */
  export const INIT: parser.GType<string> = new Type(
    "14",
  ...
  class Token implements parser.GToken<string> {
    ...
    equals(obj?: object | undefined): boolean {
      if (obj instanceof Token)
        return (
          this.lineStart == obj.lineStart &&
          this.lineEnd == obj.lineEnd &&
    ...
    hashCode32(): number {
      return utility.hashCode32(
        ...
        utility.asHashable(this.type.id),
        utility.asHashable(this.type.precedence),
        utility.asHashable(this.startPos),
        utility.asHashable(this.lineEnd),
        utility.asHashable(this.lineStart)
      );
    }
    /**
   ...
   */
  export interface MutableLexer<CH = string>
    extends parser.MutableLexer<Token, Syntax, CH> {
  ...
  export class JSONLexer implements MutableLexer<json.Value> {
    ...
    private _process(o: json.Value, s: Syntax, name = Array<Token>()) {
      if (json.isAtomic(o)) {
        if (name.length > 0) {
          ...
          this.#manufacture(
            new Token(s.sectionOperators[1], SECTION_END, 0, 0, this._i++)
          );
          this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
        }
        this.#manufacture(
          new Token(o !== null ? String(o) : "", IDENTIFIER, 0, 0, this._i++)
        );
        ...
        for (let i = 0; i < o.length; i++) {
          if (json.isAtomic(o[i])) {
            ...
            this.#manufacture(
              new Token(i.toString(), IDENTIFIER, 0, 0, this._i++)
            );
            this.#manufacture(
              new Token(s.delimiters[0], ASSIGNMENT, 0, 0, this._i++)
            );
            this.#manufacture(
              new Token(String(o[i] ?? ""), IDENTIFIER, 0, 0, this._i++)
            );
            this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
            ...
            if (json.arrayIsAtomic(array))
              for (let j = 0; j < array.length; j++) {
                ...
                this.#manufacture(
                  new Token(String(array[j] ?? ""), IDENTIFIER, 0, 0, this._i++)
                );
                this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
              }
              ...
              if (name.length < 1)
                ...
                name.push(
                  new Token(s.nesting!.chars[0], SUB_SECTION, 0, 0, this._i++)
                );
              ...
              this.#manufacture(
                new Token(JSON.stringify(o[i]), IDENTIFIER, 0, 0, this._i++)
              );
              this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
            }
        ...
        for (const key in o) {
          if (json.isAtomic(o[key])) {
            ...
            this.#manufacture(
              new Token(String(o[key] ?? ""), IDENTIFIER, 0, 0, this._i++)
            );
            this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
          } else if (Array.isArray(o[key])) {
            if (json.arrayIsAtomic(o[key] as any[]))
              for (let j = 0; j < (o[key] as any[]).length; j++) {
                this.#manufacture(new Token(key, IDENTIFIER, 0, 0, this._i++));
                this.#manufacture(
                  new Token(s.delimiters[0], ASSIGNMENT, 0, 0, this._i++)
                );
              ...
              this.#manufacture(
                new Token(JSON.stringify(o[key]), IDENTIFIER, 0, 0, this._i++)
              );
              this.#manufacture(new Token("\n", EOL, 0, 0, this._i++));
            }
    ...
    process(chunk: json.Value, syntax: Syntax, p: any): void {
      this.src = chunk;
      if (chunk === null)
        this.#manufacture(new Token("", IDENTIFIER, 0, 0, this._i++));
      else if (typeof chunk === "boolean")
        this.#manufacture(
          new Token(chunk ? "true" : "false", IDENTIFIER, 0, 0, this._i++)
        );
      else if (typeof chunk === "number")
        this.#manufacture(
          new Token(String(chunk), IDENTIFIER, 0, 0, this._i++)
        );
      else if (typeof chunk === "string")
        this.#manufacture(new Token(chunk, IDENTIFIER, 0, 0, this._i++));
    ...
    process(chunk: string = "", syntax: Syntax, p: Params): void {
      ...
      while (this.src.length > 0) {
        ...
        if (!this.#escEven()) {
          ...
          this.#manufacture(
            new Token(token, EOL, this.#ln, this.#ln, this.#li - token.length)
          );
          this.#escText = "";
          this.#esc = 0;
          ...
          this.#manufacture(
            new Token(
              ...
              this.#ln,
              this.#ln,
              this.#li - token.length
            )
          );
          this.#escText = "";
          this.#esc = 0;
        } else if (
          utility.isValid(syntax.escape) &&
          syntax.escape!.char === token
        ) {
          //an escape character?
          this.#esc++;
          if (this.#text.length > 0) {
          ...
          this.#manufacture(
            new Token(
              token,
              QUOTE.equals(this.#quoteType) ? QUOTE_END : QUOTE,
              this.#ln,
              this.#ln,
              this.#li - token.length
            )
          ...
          // this.#qt = !this.#qt;
          if (QUOTE.equals(this.#quoteType))
            this.#quoteType = null as any as Type;
          ...
          this.#manufacture(
            new Token(
              token,
              D_QUOTE.equals(this.#quoteType) ? D_QUOTE_END : D_QUOTE,
              this.#ln,
              this.#ln,
              this.#li - token.length
            )
          ...
          // console.log({lastInsertedToken: this.#queue[this.#queue.length - 1]});
          if (D_QUOTE.equals(this.#quoteType))
            this.#quoteType = null as any as Type;
          ...
          utility.isValid(syntax.nesting) &&
          syntax.nesting!.chars.indexOf(token) > -1
        ) {
          if (this.#text.length > 0) {
            this.#manufacture(
              new Token(
  ...
  class Assign extends LeftAndRight {
    parse(
      ...
      try {
        const rightExpr = p.match(EOL, l, s, pa)
          ? (null as any as Expression)
          : p.parseWithPrecedence(yp.type.precedence, l, s, pa!);
        switch (this.direction) {
          case parser.Direction.PREFIX: {
            if (utility.isValid(rightExpr) && !(rightExpr instanceof Text))
              throw new parser.ParseError(
                `Could not parse whatever was after the assignment operator at line ${yp.lineStart}, position ${yp.startPos}`
              );
      ...
      return new KeyValue(
        { preceding: Object.freeze(preceding), inline: pa!.inline },
        left,
        right
      );
    }
  ...
  class ParseSection implements Command {
    parse(
      ...
      if (
        utility.isValid(s.nesting) &&
        s.nesting!.relative &&
        p.match(SUB_SECTION, l, s, pa)
      ) {
        do {
      ...
      const scope = new Section(
        { preceding: Object.freeze([...pa!.block]), inline: pa!.inline },
        pa!.section
      );
      pa!.block = [];
      pa!.inline = "";
      while (!p.match(SECTION_START, l, s, pa!) && !p.match(EOF, l, s, pa!)) {
        ...
        pa!.assigned = false;
        if (!utility.isValid(prop)) {
          //a blank or comment line
          continue;
        } else if (prop instanceof KeyValue || prop instanceof Text) {
          //see KeyValue class docs
  ...
  class ParseText implements Command {
    parse(
      ...
      if (yp.type.equals(QUOTE)) {
        while (true) {
          if (p.match(IDENTIFIER, l, s, pa)) {
            let val = p.consume(IDENTIFIER, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(SUB_SECTION, l, s, pa!)) {
            let val = p.consume(SUB_SECTION, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(ASSIGNMENT, l, s, pa!)) {
            let val = p.consume(ASSIGNMENT, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(COMMENT, l, s, pa!)) {
            let val = p.consume(COMMENT, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(SECTION_START, l, s, pa!)) {
            let val = p.consume(SECTION_START, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(SECTION_END, l, s, pa!)) {
            let val = p.consume(SECTION_END, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(WHITESPACE, l, s, pa!)) {
            let val = p.consume(WHITESPACE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(D_QUOTE, l, s, pa)) {
            let val = p.consume(D_QUOTE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(D_QUOTE_END, l, s, pa)) {
            let val = p.consume(D_QUOTE_END, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(ESCAPE, l, s, pa)) {
            ...
            const e = new parser.ParseError(
              `Lexer is in an unsupported state. Parser matched quotes twice on ${text}`
            );
            throw new parser.SyntaxError(yp, e);
          } else {
            // console.log(`p.match(EOL) = ${p.match(EOL,l,s,pa)}`);
            const e = new parser.ParseError(
              "was unable to match the next token"
            );
            throw new parser.SyntaxError(yp, e);
          }
        }
        ...
        while (true) {
          if (p.match(IDENTIFIER, l, s, pa)) {
            ...
            let val = p.consume(QUOTE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(QUOTE_END, l, s, pa)) {
            let val = p.consume(QUOTE_END, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(ESCAPE, l, s, pa)) {
            const esc = p.consume(ESCAPE, l, s, pa);
            ...
            const e = new parser.ParseError(
              `Lexer is in an unsupported state. Parser matched double quotes twice on ${text}`
            );
            throw new parser.SyntaxError(yp, e);
          } else {
            // console.log(`p.match(EOL) = ${p.match(EOL,l,s,pa)}`);
        ...
        while (true) {
          if (p.match(IDENTIFIER, l, s, pa!)) {
            val = p.consume(IDENTIFIER, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(WHITESPACE, l, s, pa!)) {
            val = p.consume(WHITESPACE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(SUB_SECTION, l, s, pa!) && !pa!.insideSecName) {
            val = p.consume(SUB_SECTION, l, s, pa).value;
            text += val;
            src += val;
          } else if (
            (p.match(ASSIGNMENT, l, s, pa!) && pa!.assigned) ||
            (p.match(ASSIGNMENT, l, s, pa) && pa!.insideSecName)
          ) {
            val = p.consume(ASSIGNMENT, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(COMMENT, l, s, pa!) && !s.comments.inline) {
            val = p.consume(COMMENT, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(QUOTE, l, s, pa!)) {
            val = p.consume(QUOTE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(QUOTE_END, l, s, pa!)) {
            val = p.consume(QUOTE_END, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(D_QUOTE, l, s, pa!)) {
            val = p.consume(D_QUOTE, l, s, pa).value;
            text += val;
            src += val;
          } else if (p.match(D_QUOTE_END, l, s, pa!)) {
            val = p.consume(D_QUOTE_END, l, s, pa).value;
            text += val;
            src += val;
          } else if (
            p.match(ESCAPE, l, s, pa!) &&
            utility.isValid(s.escape) &&
            s.escape!.nonQuotedEsc
          ) {
            const esc = p.consume(ESCAPE, l, s, pa);
            const escaped = p.consume(ESCAPED, l, s, pa);
            val = "";
  ...
  class KeyValue implements Expression {
    ...
    debug(): string {
      return `${
        this.comments.preceding.length > 0 ? unwrapComments(this.comments) : ""
      }${this.key} = ${this.value} ${
        this.comments.inline ? ";" + this.comments.inline : ""
      }\n`;
    }
    equals(obj?: object | undefined): boolean {
    ...
    hashCode32(): number {
      return utility.hashCode32(
        false,
        utility.asHashable(this.key),
        utility.asHashable(this.value),
        utility.asHashable(this.comments)
      );
    }
    toString(): string {
  ...
  class Section implements Expression {
    ...
    constructor(
      ...
      public readonly fullname: readonly string[] = Object.freeze([])
    ) {
      this._map = {};
    }
    private static _addProp(
      section: Section,
      s: Syntax,
      names: string[],
      value: KeyValue
      ...
      if (names.length === 0) {
        ...
        if (!utility.isValid(section._map[key]))
          ...
          throw new expression.ExpressionError(
            `The property '${key}' is not of Property type. A name conflict was found`
          );
      ...
      if (!utility.isValid(section._map[name]))
        ...
        throw new expression.ExpressionError(
          `The section '${name}' is not of Section type. A name conflict was found`
        );
      return Section._addProp(section._map[name] as Section, s, names, value);
    }
    private static _addSec(
      section: Section,
      s: Syntax,
      names: string[],
      value?: Section
      ...
      if (names.length === 1) {
        ...
        switch (s.duplicateDirective.section) {
                ...
                for (const key in value!._map) {
                  ...
                  if (element instanceof Property) {
                    for (let i = 0; i < element.values.length; i++)
                      (section._map[name] as Section).add(
                        s,
                        [key],
                        element.values[i]
                      );
          ...
          case DuplicateDirective.OVERWRITE:
            break;
          case DuplicateDirective.DISCARD:
            if (utility.isValid(section._map[name])) return;
            else break;
          case DuplicateDirective.THROW:
            throw new expression.ExpressionError(
              "Duplicate not supported for section '" + name + "'"
            );
        }
    ...
    public add(s: Syntax, name: unknown, e?: unknown) {
      if (Array.isArray(name)) {
        if (e instanceof KeyValue) Section._addProp(this, s, name, e);
    ...
    private _debug(name = "", section: Section = this): string {
      ...
      for (const key in section._map) {
        if (section._map[key] instanceof Section) {
          return section._debug(
            name.length > 0 ? `${name}.${key}` : `[${key}`,
            section._map[key] as Section
          );
        } else if (section._map[key] instanceof Property) {
          rv += `${name}]${
            this.comments.inline && this.comments.inline.length > 0
              ? this.comments.inline
              : ""
          }\n${(section._map[key] as Property).debug()}`;
        } else
          throw new expression.ExpressionError(
            `Illegal value found at ${name}.${key}`
          );
      }
      return rv;
    }
    debug(): string {
      return (
        (this.comments.preceding.length > 0
          ? unwrapComments(this.comments)
          : "") + this._debug()
      );
    }
    equals(obj?: object | undefined): boolean {
    ...
    hashCode32(): number {
      return utility.hashCode32(
        false,
        utility.asHashable(this._map),
        utility.asHashable(
          this.comments
        ) /*utility.asHashable(utility.hashCode32ForArray(false, this.#props))*/
      );
    }
  ...
  class Property implements Expression {
      ...
      if (param instanceof KeyValue) {
        switch (s.duplicateDirective.property) {
          ...
          case DuplicateDirective.THROW:
            if (this._values.length > 0)
              throw Error(
                "Duplicate not supported for property '" + param.key + "'"
              );
        }
    ...
    debug(): string {
      return `${
        this.comments.preceding.length > 0 ? unwrapComments(this.comments) : ""
      }${this._values.map((x) => x.debug()).join("")}`;
    }
    equals(obj?: object | undefined): boolean {
    ...
    hashCode32(): number {
      return utility.hashCode32(
        false,
        utility.asHashable(this.comments),
        utility.asHashable(this._values)
      );
    }
    toString(): string {
      return JSON.stringify(this);
  ...
  class Text implements Expression {
    ...
    format(
      ...
      format.append(
        !utility.isValid(this.src) ? this.text : this.src!,
        syntax,
        params
      );
    }
  ...
  export class StringFormat implements Format<string> {
    ...
    constructor() {
      ...
      this.logger.seal = (l) => {
        ...
        if (l === 0) {
          ((this.logger as any)._bit as number) =
            0x1 & ((this.logger as any)._bit as number);
          this.logger.error = (m) => {
            throw Error("Sealed");
          };
        } else if (l === 1) {
          ((this.logger as any)._bit as number) =
            0x2 & ((this.logger as any)._bit as number);
          this.logger.info = (m) => {
            throw Error("Sealed");
          };
        } else if (l === 2) {
          ((this.logger as any)._bit as number) =
            0x4 & ((this.logger as any)._bit as number);
          this.logger.warn = (m) => {
            throw Error("Sealed");
          };
        }
    ...
    append(
      ...
      if (typeof data === "string") {
        ...
        this.append(
          data.comments.preceding.length > 0
            ? unwrapComments(data.comments, s) + "\n"
            : "",
          s,
          p
        );
        if (data.value.length > 0)
          this.append(
            `${processEscapables(data.key, s!, this)} ${
              s!.delimiters[0]
            } ${processEscapables(data.value, s!, this)}`,
            s,
            p
          );
        else this.append(`${processEscapables(data.key, s!, this)}`, s, p);
        this.append(
          `${data.comments.inline ? data.comments.inline : ""}`,
          s,
          p
        );
  ...
  export class JSFormat implements Format<json.Value> {
      ...
      for (const key in data.map) {
        if (data.map[key] instanceof Section) {
          ...
          if (prop.values.length < 2) {
            rv[key] =
              prop.values.length === 0 ? null : s?.parse(prop.values[0].value);
          } else {
            rv[key] = Array<string>();
    ...
    append(
      ...
      /*if(typeof data === "string"){
        ...
        throw new expression.FormatError("format not supported", data as any);
    }
    data(): json.Pair {
      return this._data;
    }
